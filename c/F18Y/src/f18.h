...
//
// sizeof(Node) = 652 bytes (update me now and then)
...
typedef struct Node {
    ...
    uint18_t (*read_ioreg)(struct Node* np, uint18_t reg);
    void     (*write_ioreg)(struct Node* np, uint18_t reg, uint18_t val);
