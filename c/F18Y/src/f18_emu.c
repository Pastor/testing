...
static uint18_t read_mem(Node *np, uint18_t addr) {
    uint18_t value;
    if (addr <= RAM_END2) {
...
static void write_mem(Node *np, uint18_t addr, uint18_t val) {
    if (addr <= RAM_END2) {
