...
int select_ports(Node *np, uint18_t ioreg, int is_input,
...
static void f18_write_ioreg(Node *np, uint18_t ioreg, uint18_t value) {
    uint18_t io_wr[4];
    int fds[4];
    int nports;
    int i, r;
...
int parse_mnemonic(char *word, int n) {
    int i;
    for (i = 0; i < 32; i++) {
        int len = strlen(f18_instruction_name[i]);
...
static uint18_t f18_read_ioreg(Node *np, uint18_t ioreg) {
    uint18_t io_rd[4];
    uint18_t dd = 0;
    int fds[4];
    int fd;
