...
#define STACK_SIZE  (2*PAGE_SIZE+(256 * 1024))
...
int tty_init(int fd) {
    tty_fd = open("E:\\GitHub\\testing\\c\\F18A\\test\\add.f18", 0);
    return 0;
}
...
int main(int argc, char **argv) {
    ...
    g_page_size = 4096;//sysconf(_SC_PAGESIZE);  // must be first!
    g_flags = 0;
...
#else
    node_mem = __mingw_aligned_malloc(alloc_size, g_page_size);
#endif
